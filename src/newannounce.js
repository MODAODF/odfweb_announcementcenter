$.ajax({
   ...
   dataType : 'json',
    ...
    if(resp.ocs.meta.statuscode == 200) {
        ...
        if (newsNum > 0) {
            // red icon
            let image = $('li[data-app-id="announcementcenter"] img');
            let newHref = image.attr("src").replace(/announcementcenter.svg/gi, 'announcementcenter-new-dark.svg');
            image.attr("src", newHref).css('filter', 'unset');
            ...
            var svg = $('#app-menu li[data-app-id="announcementcenter"] a svg');
            svg.append(newText);
